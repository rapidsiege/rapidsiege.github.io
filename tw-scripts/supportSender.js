...
// custom version: all traffic outside the Tribalwars game removed
// (counterapi.dev usage tracking, Dropbox-hosted CSS now inlined, icons8 images
//  replaced with emoji, unit icons from the world's own /graphic/ path)
// + Defensive-plan import: paste the per-player "📜 Export supportSender" text from
//   tribe-calculator and fill each planned mass-support order target by target.
//   Run via quickbar on the Mass Support screen; the loaded plan, done-marks and
//   village-ID cache survive in localStorage, so after navigating to the next
//   target just run the script again and everything is restored.
...
var ssRightScreen = url.includes("screen=place&mode=call")
if(!ssRightScreen)
{
...
// inlined replacement for the Dropbox-hosted styleCSSGlobal.js
// (only the classes this script uses; built from the same theme variables)
...
function createMainInterface(){
    ...
    for(let i=0;i<units.length;i++){
        if(units[i]!="knight" && units[i]!="snob" && units[i]!="militia" && units[i]!="axe" && units[i]!="light" && units[i]!="ram" && units[i]!="catapult" && units[i]!="marcher"){
            html+=`<td class="fm_unit"><img src="/graphic/unit/unit_${units[i]}.webp"></td>`
        }
    }
    html+=`
                    <td >pop</td>
...
// ═══════════════════════════════════════════════════════════════════════════
// Defensive-plan import (tribe-calculator "📜 Export supportSender")
// ⚠ FORMAT CONTRACT with defScriptText() in tribe-calculator js/defense-plan.js —
...
// The player pastes their per-player export once; it is stored per world. On each
...
// ── pure helpers (tested headless in .omc/test_support_sender.js) ──
