// ══════════════════════════════════════════════════════════════
// CHANGELOG ARCHIVE — v5.x and older (frozen history)
// ──────────────────────────────────────────────────────────────
// Split out of changelog.js (2026-07-24) so the file touched on EVERY release
// stays small: this one holds v5.17.1 down to v1.4.0 (v5.x added 2026-09-24 when
// v6.0.0 landed) and should never need editing again. New entries go at the TOP of CHANGELOG_CURRENT in
// changelog.js, which concatenates this array after its own.
//
// Loaded as a classic <script src> BEFORE js/changelog.js (file:// safe, no
// modules/fetch). If this file is ever missing, changelog.js degrades to
// showing only the current major instead of breaking (typeof guard there).
//
// Same entry shape as CHANGELOG_CURRENT:
//   { ver, date, tagEn?, tagEs?, en:[<li> inner HTML…], es:[…] }
// ══════════════════════════════════════════════════════════════
const CHANGELOG_ARCHIVE = [
  { ver: 'v5.17.1', date: '2026-09-21',
    en: [
      `<b>🌍 World-aware defaults.</b> On a fresh install the rally/info <b>Server URL</b> now follows the selected <b>World</b> instead of always starting at es100, and the hidden-tribe rule in Enemy Villages (the tool never shows report intel about the tribe's own villages) is now defined per world — es103 got its own list. The hosted copy can also be pointed at another site's world-data mirror, which is how a per-tribe private copy is built.`,
    ],
    es: [
      `<b>🌍 Valores por defecto según el mundo.</b> En una instalación nueva la <b>URL del servidor</b> del punto de reunión sigue ahora el <b>Mundo</b> seleccionado en vez de empezar siempre en es100, y la regla de tribu oculta de Aldeas Enemigas (la herramienta nunca muestra inteligencia de informes sobre las aldeas de la propia tribu) se define ahora por mundo — es103 tiene su propia lista. La copia alojada también puede apuntar a la réplica de datos de mundo de otro sitio, que es como se construye una copia privada por tribu.`,
    ],
  },
  ...
  { ver: 'v5.16.0', date: '2026-09-09',
    en: [
      ...
      `<b>✏ Edit Selected Rows: mass-edit the noble train per group.</b> A new <b>Nobles</b> row sets <b>Snob Players</b> and <b>Nobles</b> on every selected target, and the <b>Snob Mode</b> buttons next to it write the mode — both into the group chosen in the picker above them (shown when there are several groups; other groups' trains are left alone), the same way the Offs row fills one wave at a time. The old "move the noble train to a group" row is gone, since a train now simply lives in whichever groups have nobles.`,
    ],
    es: [
      `<b>👑 Trenes de nobles por grupo de ventana.</b> Hasta ahora un objetivo tenía un solo tren de nobles y elegía la oleada en la que caía; las celdas <b>Jug. Nobles</b>, <b>Nobles</b>, <b>Remitentes</b> y <b>Modo Nobles</b> funcionan ahora igual que las de Completo / 3-4 / 1-2 — una entrada por grupo (A, B, C…), así que un objetivo puede recibir 4 nobles en la ventana de nobles de la oleada A <i>y</i> 2 más en la de la oleada B, cada uno con sus propios remitentes y su propio modo Solo / Partir Off / Fake. Cada tren se planifica contra la ventana de nobles y la fecha de llegada de su propio grupo, la off de coordinación del conquistador viaja en la misma oleada que el tren al que abre camino (una por oleada) y las exportaciones agrupan los trenes bajo la FECHA DE LLEGADA de su oleada como las offs. La columna <b>Ventanas Nobles</b> muestra las oleadas en las que se noblea el objetivo. Los planes de un solo grupo se ven y se comportan exactamente igual que antes; los objetivos existentes conservan su tren en la oleada que tenían asignada.`,
      `<b>✏ Editar filas seleccionadas: edición masiva del tren de nobles por grupo.</b> Una nueva fila <b>Nobles</b> fija <b>Jug. Nobles</b> y <b>Nobles</b> en todos los objetivos seleccionados, y los botones de <b>Modo Nobles</b> junto a ella escriben el modo — ambos en el grupo elegido en el selector de encima (visible cuando hay varios grupos; los trenes de los demás grupos no se tocan), igual que la fila de Offs rellena una oleada a la vez. La antigua fila «mover el tren de nobles a un grupo» desaparece: un tren vive ahora, simplemente, en los grupos que tengan nobles.`,
    ],
  },
  ...
  { ver: 'v5.14.0', date: '2026-08-29',
    en: [
      ...
      `<b>🔍 Row filter (Offensive Targets).</b> A new <b>🔍 Filter</b> button opens a bar with a text search (coordinate or defender), a <b>points condition</b> (e.g. <b>&lt; 3000</b>) and a <b>target type</b>. Only matching rows are shown — the button lights up and a "Showing X of Y targets" note appears. It is a view filter: nothing is removed from the list, the plan and exports still see every target, but <b>select-all and Edit Selected Rows act on the visible rows only</b>, so you can filter to "&lt; 3000 points", select all and mass-edit exactly those. Remembered on this device; ✕ Clear resets it.`,
      `<b>📅 Window Groups: typing a date or time really keeps focus now.</b> v5.12.4 stopped the date editor from rebuilding the group rows, but the table refresh it triggers rebuilt them anyway, so the box still lost focus after the first digit — for the time inputs too. The editors now refresh the table while leaving the group rows in place.`,
    ],
    es: [
      `<b>↕ Objetivos Ofensivos ordenables.</b> Haz clic en la cabecera <b>#</b>, <b>Coord</b>, <b>Defensor</b> o <b>Puntos</b> para ordenar la tabla (otro clic invierte; Puntos empieza de mayor a menor). Ordenar solo cambia la vista: el <b>#</b> sigue siendo el número de objetivo de cada fila — el #3 sigue siendo el #3 caiga donde caiga — y el orden de la lista que usan el plan, las exportaciones y las órdenes por jugador no cambia. El orden se recuerda en este dispositivo.`,
      `<b>🔍 Filtro de filas (Objetivos Ofensivos).</b> Un nuevo botón <b>🔍 Filtrar</b> abre una barra con búsqueda de texto (coordenada o defensor), una <b>condición de puntos</b> (p. ej. <b>&lt; 3000</b>) y un <b>tipo de objetivo</b>. Solo se muestran las filas que cumplen — el botón se ilumina y aparece «Mostrando X de Y objetivos». Es un filtro de vista: no se quita nada de la lista, el plan y las exportaciones siguen viendo todos los objetivos, pero <b>seleccionar todo y Editar Filas Seleccionadas actúan solo sobre las filas visibles</b>, así puedes filtrar «&lt; 3000 puntos», seleccionar todo y editar en masa exactamente esas. Se recuerda en este dispositivo; ✕ Limpiar lo restablece.`,
      `<b>📅 Grupos de ventanas: escribir una fecha u hora ya mantiene el foco de verdad.</b> La v5.12.4 evitó que el editor de fecha reconstruyera las filas de grupo, pero el refresco de la tabla que dispara las reconstruía igualmente, así que la caja seguía perdiendo el foco tras el primer dígito — también en las horas. Los editores ahora refrescan la tabla dejando las filas de grupo en su sitio.`,
    ],
  },
  ...
  { ver: 'v3.27.0', date: '2026-07-04',
    ...
    es: [
      `<b>🎯 Dibuja el Filtro de Coordenadas en el mapa.</b> La pestaña Mapa tiene un nuevo botón <b>🎯 Dibujar Filtro de Coordenadas</b>: haz clic para poner puntos y trazar un <b>área</b> en el mapa (3+ puntos cierran una forma; una línea discontinua previsualiza el siguiente lado). Al Generar un plan, los offs y los trenes de nobles se envían <b>solo desde aldeas dentro de esa forma</b> — la versión dibujada del Filtro de Coordenadas escrito. Una pequeña barra muestra el número de puntos con <b>↶ Deshacer punto</b> y <b>✕ Borrar área</b>; al hacer clic de nuevo se <b>amplía</b> la forma actual. El área se dibuja en <b>coordenadas del mapa</b>, así que se mantiene al mover o hacer zoom, y se guarda con tu plan. Además se <b>combina con el filtro X/Y escrito</b> (una aldea debe cumplir ambos), y el panel Plan Ofensivo → Filtro de Coordenadas ahora indica cuándo hay un área del mapa activa (con su propio Borrar). Si un jugador no tiene ninguna aldea dentro del área que pueda enviar un noble, sigue apareciendo el aviso habitual de <b>"sin aldeas en alcance de noble"</b>.`,
    ],
  },
  { ver: 'v3.26.0', date: '2026-07-04',
    en: [
      `<b>🎯 Coordinate Filter in Plan Offensive.</b> A new <b>Coordinate Filter</b> button (under the Generate Plan controls) lets you restrict which of your own villages are used as senders by their <b>map position</b>. Add one or more filters — pick <b>X</b> or <b>Y</b>, an operator (<b>&gt; ≥ &lt; ≤ =</b>) and a number — and they <b>stack (AND)</b>: e.g. <i>X &gt; 500</i> and <i>Y &gt; 500</i> only sends from villages in the bottom-right of the map. When you Generate a plan, both <b>clearing offs and noble (snob) trains</b> are drawn only from villages that pass every filter. If a player has no in-filter village that can send a noble, the usual <b>"no villages in snob range"</b> notice still shows. Leave the filter empty (or half-typed) to use every village as before — the setting is saved with your plan.`,
    ],
    es: [
      `<b>🎯 Filtro de Coordenadas en Plan Ofensivo.</b> Un nuevo botón <b>Filtro de Coordenadas</b> (bajo los controles de Generar Plan) te permite limitar qué aldeas propias se usan como remitentes según su <b>posición en el mapa</b>. Añade uno o varios filtros — elige <b>X</b> o <b>Y</b>, un operador (<b>&gt; ≥ &lt; ≤ =</b>) y un número — y se <b>combinan (Y)</b>: p. ej. <i>X &gt; 500</i> e <i>Y &gt; 500</i> solo envía desde aldeas de la parte inferior derecha del mapa. Al Generar un plan, tanto los <b>offs de limpieza como los trenes de nobles</b> se toman únicamente de aldeas que cumplen todos los filtros. Si un jugador no tiene ninguna aldea dentro del filtro que pueda enviar un noble, sigue apareciendo el aviso habitual de <b>"sin aldeas en alcance de noble"</b>. Deja el filtro vacío (o a medio escribir) para usar todas las aldeas como antes — el ajuste se guarda con tu plan.`,
    ],
  },
  { ver: 'v3.25.0', date: '2026-07-03',
    en: [
      `<b>☑ Select rows &amp; mass-edit in Offensive Targets.</b> Every target row now has a <b>checkbox</b> on the far left (selected rows highlight light grey; the header checkbox selects/deselects all). With rows selected, the new <b>✏ Edit Selected Rows</b> button (top right, next to Ignore Coordinates) opens a popup that edits <b>all selected rows at once</b>: turn <b>POWER</b> on/off, turn <b>Catapults</b> on/off with the <b>number of attacks</b>, set the <b>off counts</b> (Complete / 3-4 / 1-2), set the <b>Snob Mode</b> (Solo / Split Off), set the <b>Catapult Mode</b> building (Smithy / Farm / Wall), replace the <b>off windows</b> and the <b>snob window</b>, or <b>delete all selected rows</b> (with a confirmation). The Catapults row can also assign the <b>target buildings and their counts</b> (turning Catapults on for the rows), the Off Windows editor supports <b>several windows</b> (with per-window counts) just like the table cell, and every row carries its <b>unit icon</b>. The popup stays open so you can chain several edits on the same selection.`,
      `<b>📊 Clearer "Offs assigned" summary.</b> The line under the table now shows <b>assigned / available</b> per off tier, where <b>available</b> already excludes the villages held out by <b>Ignore Coordinates</b> and <b>Ignore Players</b> (a note says how many were excluded). If you assign more offs than are available, the number turns <b>red</b>. The line updates live as you edit the count cells or the ignore list, and a new <b>↻</b> button recalculates it on demand. Hover it for the fine print — distance, morale and noble-launch holds only apply when the plan is generated (the Plan tab's footer shows that final breakdown).`,
    ],
    es: [
      `<b>☑ Selecciona filas y edítalas en masa en Objetivos Ofensivos.</b> Cada fila de objetivo tiene ahora una <b>casilla</b> a la izquierda del todo (las filas seleccionadas se resaltan en gris claro; la casilla de la cabecera selecciona/deselecciona todas). Con filas seleccionadas, el nuevo botón <b>✏ Editar Filas Seleccionadas</b> (arriba a la derecha, junto a Ignorar Coordenadas) abre una ventana que edita <b>todas las filas seleccionadas a la vez</b>: activar/desactivar <b>POWER</b>, activar/desactivar <b>Catapultas</b> con la <b>cantidad de ataques</b>, fijar las <b>cantidades de offs</b> (Completo / 3-4 / 1-2), fijar el <b>Modo Nobles</b> (Solo / Partir Off), fijar el edificio del <b>Modo Catapulta</b> (Herrería / Granja / Muralla), reemplazar las <b>ventanas de off</b> y la <b>ventana de nobles</b>, o <b>eliminar todas las filas seleccionadas</b> (con confirmación). La fila de Catapultas también permite asignar los <b>edificios objetivo y sus cantidades</b> (activando Catapultas en las filas), el editor de Ventanas Off admite <b>varias ventanas</b> (con cantidades por ventana) igual que la celda de la tabla, y cada fila lleva su <b>icono de unidad</b>. La ventana permanece abierta para encadenar varias ediciones sobre la misma selección.`,
      `<b>📊 Resumen "Offs asignados" más claro.</b> La línea bajo la tabla ahora muestra <b>asignados / disponibles</b> por nivel de off, donde <b>disponibles</b> ya excluye las aldeas apartadas por <b>Ignorar Coordenadas</b> e <b>Ignorar Jugadores</b> (una nota indica cuántas se excluyeron). Si asignas más offs de los disponibles, el número se pone <b>rojo</b>. La línea se actualiza en vivo al editar las celdas de cantidades o la lista de ignorados, y un nuevo botón <b>↻</b> la recalcula bajo demanda. Pasa el ratón por encima para la letra pequeña — la distancia, la moral y las reservas de lanzamiento de nobles solo se aplican al generar el plan (el pie de la pestaña Plan muestra ese desglose final).`,
    ],
  },
  ...
  { ver: 'v3.21.0', date: '2026-07-01',
    en: [
      ...
      `<b>🎯 "Catapult Mode" column — the building your off senders aim at.</b> A new dropdown right of Snob Mode sets the <b>building objective for this target's off senders</b> (the catapults riding with the clearing/destroyer offs and the offs that escort a noble) — <b>Smithy, Farm or Wall</b>, default <b>Smithy</b>. Turning on <b>POWER</b> forces it to <b>Wall</b> (and locks it); turning POWER off restores your previous choice — and if you'd already set Wall yourself, it stays Wall. This is separate from the per-attack buildings in the Catapults column (those only steer the extra catapult attacks sent from your defensive villages). The objective shows in the plan and exports, and the off attack links pre-select it where the game allows.`,
    ],
    es: [
      `<b>🏰 Dirige los ataques de catapulta a edificios concretos.</b> Cuando marcas <b>Catapultas</b> en un Objetivo Ofensivo, ahora puedes elegir <b>qué edificios derribar</b> debajo de la casilla de número de ataques — elige entre <b>Herrería, Granja, Leñador, Barrera y Mina de Hierro</b>, igual que al asignar remitentes de noble. Cada edificio tiene su casilla de número: déjala en <b>0</b> y los ataques se reparten equitativamente entre los edificios elegidos — y si se pueden enviar menos ataques de los pedidos, se reparten por turnos (p. ej. 5 pedidos en 3 edificios pero solo 3 enviados → 1/1/1, no 2/1/0). Pon un número para fijar exactamente esa cantidad a un edificio. Con un solo edificio, todos los ataques van a él. El edificio objetivo aparece en el plan y en todas las exportaciones, y el enlace de ataque lo preselecciona cuando el juego lo permite.`,
      `<b>🎯 Columna "Modo Catapulta" — el edificio al que apuntan tus remitentes de off.</b> Un nuevo desplegable a la derecha de Modo Noble fija el <b>edificio objetivo para los remitentes de off de este objetivo</b> (las catapultas que acompañan a los offs de limpieza/destructores y a los offs que escoltan un noble) — <b>Herrería, Granja o Muralla</b>, por defecto <b>Herrería</b>. Al activar <b>POWER</b> se fuerza a <b>Muralla</b> (y se bloquea); al desactivar POWER se restaura tu elección anterior — y si ya habías puesto Muralla tú, se queda en Muralla. Es independiente de los edificios por-ataque de la columna Catapultas (esos solo dirigen los ataques extra de catapulta enviados desde tus aldeas defensivas). El objetivo aparece en el plan y las exportaciones, y los enlaces de ataque off lo preseleccionan cuando el juego lo permite.`,
    ],
  },
